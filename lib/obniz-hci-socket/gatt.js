...
Gatt.prototype._runNextCommand = function() {

  if (this._currentCommand === null) {
    while (this._commandQueue.length) {
      this._currentCommand = this._commandQueue.shift();
      if (this._currentCommand.commandType === "command") {
        this.writeAtt(this._currentCommand.buffer);
      } else if (this._currentCommand.commandType === "encrypt") {
...
Gatt.prototype.onAclStreamEncrypt = function(encrypt) {
  if (encrypt) {
    this._security = 'medium';
    if(this._currentCommand && this._currentCommand.cmmandType === "command"){
      this.writeAtt(this._currentCommand.buffer);
    }else if(this._currentCommand && this._currentCommand.cmmandType === "encrypt"){
      if(this._currentCommand.callback){
...
Gatt.prototype.encrypt = function(options){
    this._queueCommand(null, function(){}, null, "encrypt");
};

Gatt.prototype._queueCommand = function(buffer, callback, writeCallback, commandType, options) {
  if(!commandType){
    commandType = "command";
  }
