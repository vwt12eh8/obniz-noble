...
NobleBindings.prototype.onLeConnComplete = function(status, handle, role, addressType, address, interval, latency, supervisionTimeout, masterClockAccuracy) {
  ...
  if (status === 0) {
    ...
    this._aclStreams[handle].on("encryptFail", this.onPairingFailed.bind(this));
    this._aclStreams[handle].on("pairing", this.onPairing.bind(this));
...
NobleBindings.prototype.onPairing = function(handle, keys) {
  var peripheralUuid = this._handles[handle];
  this.emit('pairing', peripheralUuid, null, keys);
}

NobleBindings.prototype.onPairingFailed = function(handle, error) {
  var peripheralUuid = this._handles[handle];
  this.emit('pairing', peripheralUuid,  error);
}
