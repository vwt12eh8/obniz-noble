...
function Noble(bindings) {
  ...
  this._bindings.on('pairing', this.onPairing.bind(this));
...
Noble.prototype.pairing = function(peripheralUuid,options) {
  this._bindings.pairing(peripheralUuid,options);
};

Noble.prototype.onPairing = function(peripheralUuid, error, keys) {
  var peripheral = this._peripherals[peripheralUuid];
  peripheral.emit("pairing", error, keys);
};
